...
class TextRenderingScene {
	...
	void onCharacterInput(char codepoint) {
		...
		for (int i = 0; i < 4; ++i) {
			text_renderers[i]->text = TEXT.data();
			text_renderers[i]->text_length = TEXT.size();
			text_renderers[i]->buildMesh();
		}
...
public:
	...
	TextRenderingScene() {
		...
		for (int i = 0; i < 4; ++i) {
			...
			this->text_renderers[i] = &text_renderer;

			switch (i) {
				case 0:
					text_renderer.alignment = TEXT_ALIGNMENT_CENTER;
					text_renderer.pivot = PIVOT_BOTTOM_RIGHT;
					text_entity.get<Transform>().translate(vec3(-1.0f, 1.0f, 0.0f));
					break;
				case 1:
					text_renderer.alignment = TEXT_ALIGNMENT_LEFT;
					text_renderer.pivot = PIVOT_BOTTOM_LEFT;
					text_entity.get<Transform>().translate(vec3(1.0f, 1.0f, 0.0f));
					break;
				case 2 :
					text_renderer.alignment = TEXT_ALIGNMENT_CENTER;
					text_renderer.pivot = PIVOT_TOP_RIGHT;

					text_entity.get<Transform>().translate(vec3(-1.0f, -1.0f, 0.0f));
					break;
				case 3:
					text_renderer.alignment = TEXT_ALIGNMENT_RIGHT;
