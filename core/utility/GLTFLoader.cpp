...
namespace HBE {
	void processNodes(ModelData &data, tinygltf::Model &model, const tinygltf::Node &node,
		...
		transform.setLocal(model_node.transform);

		if (node.translation.size() == 3) {
			transform.translate(vec3(node.translation[0], node.translation[1], node.translation[2]));
		}
		if (node.scale.size() == 3) {
			transform.setLocalScale(vec3(node.scale[0], node.scale[1], node.scale[2]));
		}
		if (node.rotation.size() == 4) {
			transform.setRotation(quat(node.rotation[3], node.rotation[0], node.rotation[1], node.rotation[2]));
		} else if (node.rotation.size() == 3) {
			transform.setRotation(vec3(node.rotation[0], node.rotation[1], node.rotation[2]));
		}

		model_node.transform = transform.local();
