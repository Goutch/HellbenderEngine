	...
	void processNodes(ModelData &data, tinygltf::Model &model, const tinygltf::Node &node, std::vector<ModelNode> &node_array) {
		...
		if ((node.mesh >= 0) && (node.mesh < model.meshes.size())) {
			...
			for (int i = 0; i < gltf_mesh.primitives.size(); ++i) {
				...
				HB_ASSERT(material_index >= 0 && material_index < model.materials.size(), "Material index out of bounds");
		...
		for (size_t mesh_index = 0; mesh_index < model.meshes.size(); mesh_index++) {
			...
			for (size_t primitive_index = 0; primitive_index < gltf_mesh.primitives.size(); ++primitive_index) {
				...
				if (gltf_primitive.indices != -1) {
					...
					indices_buffer.data = reinterpret_cast<const void *>(&index_buffer.data.at(index_accessor.byteOffset + index_buffer_view.byteOffset));
				...
				for (const auto &attribute: gltf_primitive.attributes) {
					...
					if (attribute_name == "POSITION") {
						...
						primitive.position_min = vec3(accessor.minValues[0], accessor.minValues[1], accessor.minValues[2]);
						primitive.position_max = vec3(accessor.maxValues[0], accessor.maxValues[1], accessor.maxValues[2]);
					...
					const void *data_ptr = reinterpret_cast<const void *>(&buffer.data.at(accessor.byteOffset + buffer_view.byteOffset));
	...
	void createMaterialData(ModelData &data, tinygltf::Model &model) {
		for (size_t i = 0; i < model.materials.size(); i++) {
			...
			if (gltf_material.pbrMetallicRoughness.baseColorFactor.size() == 4) {
				material_data.properties.base_color.r = static_cast<float>(gltf_material.pbrMetallicRoughness.baseColorFactor[0]);
				material_data.properties.base_color.g = static_cast<float>(gltf_material.pbrMetallicRoughness.baseColorFactor[1]);
				material_data.properties.base_color.b = static_cast<float>(gltf_material.pbrMetallicRoughness.baseColorFactor[2]);
				material_data.properties.base_color.a = static_cast<float>(gltf_material.pbrMetallicRoughness.baseColorFactor[3]);
			}
			material_data.albedo_texture = gltf_material.pbrMetallicRoughness.baseColorTexture.index;
			material_data.metallic_roughness_texture = gltf_material.pbrMetallicRoughness.metallicRoughnessTexture.index;
			...
			material_data.properties.has_metallic_roughness = gltf_material.pbrMetallicRoughness.metallicRoughnessTexture.index != -1;
			...
			material_data.properties.emmisive_factor = vec4(gltf_material.emissiveFactor[0], gltf_material.emissiveFactor[1], gltf_material.emissiveFactor[2], 0.0f);
