...
namespace HBE {
	...
	Mesh *DefaultModelParser::createMesh(const ModelPrimitiveData &data, const ModelInfo model_info) {
		...
		mesh_info.flags |= (model_info.flags & MODEL_FLAG_USED_IN_RAYTRACING) == MODEL_FLAG_USED_IN_RAYTRACING ? MESH_FLAG_USED_IN_RAYTRACING : MESH_FLAG_NONE;
		...
		if (normal_it != data.buffers.end()) {
			mesh_ptr->setBuffer(1, normal_it->second[0].data, normal_it->second[0].count);
		}
		if (uv_it != data.buffers.end()) {
			mesh_ptr->setBuffer(2, uv_it->second[0].data, uv_it->second[0].count);
		}
		return mesh_ptr;
	}

	GraphicPipelineInstance *DefaultModelParser::createMaterial(const ModelMaterialData &materialData, HBE::Texture **textures) {
		GraphicPipelineInstanceInfo instance_info{};
		instance_info.graphic_pipeline = materialData.double_sided ? parser_info.graphic_pipeline_2_sided : parser_info.graphic_pipeline;
