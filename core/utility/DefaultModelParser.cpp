...
namespace HBE {
	DefaultModelParser::DefaultModelParser(const DefaultModelParserInfo &info) : context(*Application::instance->getContext()) {
	...
	PipelineInstanceHandle DefaultModelParser::createMaterial(const ModelMaterialData &materialData,
		...
		instance_info.pipeline_handle = materialData.double_sided
		                                ? parser_info.rasterization_pipeline_2_sided
		                                : parser_info.rasterization_pipeline;
		instance_info.type = PIPELINE_INSTANCE_TYPE_RASTERIZATION;
