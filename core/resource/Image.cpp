...
namespace HBE {
    Image::Image(ImageHandle handle) : image_interface(Application::instance->getContext()->image_interface) {
        this->handle = handle;
    }

    vec3u Image::getSize() {
        return image_interface.getImageSize(handle);
    }

    void Image::release() {
        image_interface.releaseImage(handle);
    }

    void Image::update(const void *data) {
        image_interface.updateImage(handle, data);
    }
    ...
    Image *Image::load(const std::string &path, ImageInfo &info) {
        ...
        texture = Application::instance->getContext()->createImage(info);
