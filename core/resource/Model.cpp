...
namespace HBE
{
    Model::~Model()
    {
        for (ImageHandle texture : resources.textures)
        {
            context.releaseImage(texture);
        }
        for (std::vector<MeshHandle> meshes : resources.meshes)
        {
            for (int i = 0; i < meshes.size(); ++i)
            {
                context.releaseMesh(meshes[i]);
            }
        }

        for (auto material : resources.materials)
        {
            context.releasePipelineInstance(material);
        }

        for (MeshAccelerationStructureHandle acceleration_structure : resources.acceleration_structures)
        {
            context.releaseMeshAccelerationStructure(acceleration_structure);
        }
    }

    Model::Model(const ModelInfo& info) : context(*Application::instance->getContext())
    {
        this->info = info;
        load(info);
    }

    Model::Model() : context(*Application::instance->getContext())
    {
    }
    ...
    void Model::createMaterials()
    {
        ...
        {
            PipelineInstanceHandle material = info.parser->createMaterial(data.material_properties[i], resources.textures.data());
