...
namespace HBE {
	...
	VK_Allocator::VK_Allocator(VK_Device *device) {
		this->device = device;
		this->command_pool = new VK_CommandPool(*device, 8, device->getQueue(QUEUE_FAMILY_GRAPHICS));

		Graphics::onFrameChange.subscribe(on_frame_change_subscription_id, this, &VK_Allocator::processFreeRequests);
	...
	VK_Allocator::~VK_Allocator() {
		...
		Graphics::onFrameChange.unsubscribe(on_frame_change_subscription_id);
	}
	...
	void VK_Allocator::processFreeRequests(uint32_t frame) {
		...
		HB_PROFILE_END("processFreeRequests");
	}
	...
	VK_Fence *VK_Allocator::copy(VkBuffer src, VkBuffer dest, VkDeviceSize size, VkDeviceSize offset) {
		...
		return &command_pool->submit(QUEUE_FAMILY_GRAPHICS);
	}
