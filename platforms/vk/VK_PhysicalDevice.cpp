...
namespace HBE
{
    void VK_PhysicalDevice::init(VK_Instance& instance, VK_Surface& surface)
    {
        ...
        ApplicationInfo app_info = Application::instance->getInfo();
        ...
        if (app_info.context_info.required_extension_flags & VULKAN_REQUIRED_EXTENSION_RTX)
        {
        ...
        if (app_info.context_info.required_extension_flags & VULKAN_REQUIRED_EXTENSION_DESCRIPTOR_INDEXING ||
        ...
        if (app_info.context_info.vulkan_version >= VULKAN_VERSION_1_3)
        {
    ...
    void VK_PhysicalDevice::release()
    {
    }
    ...
    void VK_PhysicalDevice::pickBestPhysicalDevice()
    {
        ...
        vkEnumeratePhysicalDevices(instance->getHandle(), &device_count, available_physical_devices.data());
    ...
    bool VK_PhysicalDevice::isDeviceSuitable(VkPhysicalDevice const& physical_device)
    {
        ...
        if (app_info.context_info.required_extension_flags & VULKAN_REQUIRED_EXTENSION_RTX)
        {
            ...
            if (app_info.context_info.vulkan_version >= VULKAN_VERSION_1_2)
        ...
        if ((app_info.context_info.required_extension_flags & VULKAN_REQUIRED_EXTENSION_DESCRIPTOR_INDEXING) &&
            (app_info.context_info.vulkan_version < VULKAN_VERSION_1_2))
        {
    ...
    QueueFamilyIndices VK_PhysicalDevice::getSupportedQueueFamilies(VkPhysicalDevice const& physical_device)
    {
        ...
        for (uint32_t i = 0; i < queue_family_count; ++i)
        {
            ...
            vkGetPhysicalDeviceSurfaceSupportKHR(physical_device, i, surface->getHandle(), &present_supported);
        ...
        VkPhysicalDevice const& physical_device) const
    {
        ...
        vkGetPhysicalDeviceSurfaceCapabilitiesKHR(physical_device, surface->getHandle(), &details.capabilities);
