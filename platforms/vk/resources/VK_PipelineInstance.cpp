...
namespace HBE
{
    ...
    void VK_PipelineInstance::copyDescriptorSets(DescriptorPool& from, DescriptorPool& to, int64_t frame)
    {
        ...
        uint32_t write_index = frame * binding_count;
        for (; write_index < pipeline_layout->getDescriptorBindings().size(); write_index++)
        {
            VkWriteDescriptorSet& from_write = from.writes[write_index];
            VkWriteDescriptorSet& to_write = to.writes[write_index];
    ...
    void VK_PipelineInstance::createDescriptorWrites(DescriptorPool& pool)
    {
        ...
        for (int binding = 0; binding < pool.writes.size(); ++binding)
        {
            ...
            if (pool.writes[binding].descriptorType == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER)
            {
                ...
                buffer_infos[binding]->buffer = uniform_buffers[binding].getHandle();
    ...
    VkDescriptorSet VK_PipelineInstance::getDescriptorSetForBinding(uint32_t binding)
    {
        uint32_t frame = context->renderer.getCurrentFrameIndex();
        uint32_t descriptor_set_count = pipeline_layout->getDescriptorSetCount();
        uint32_t descriptor_index = pipeline_layout->getDescriptorInfos()[binding].descriptor_set;
    ...
    void VK_PipelineInstance::setUniform(uint32_t binding, const void* data)
    {
        ...
        {
            buffer_infos[binding]->buffer = uniform_buffers[binding_frame_index].getHandle();
        }
    ...
    void VK_PipelineInstance::setAccelerationStructure(uint32_t binding, RootAccelerationStructureHandle acceleration_structure, int32_t frame)
    {
        ...
        const VkAccelerationStructureKHR acceleration_structure_handle = vk_top_level_acceleration_structure_object.getHandle();
    ...
    void VK_PipelineInstance::setStorageBufferArray(uint32_t binding, BufferHandle* buffers, uint32_t count, int32_t frame)
    {
        ...
        {
            buffer_infos[binding][i].buffer = vk_buffers[i]->getBuffer().getHandle();
    ...
    void VK_PipelineInstance::setStorageBuffer(uint32_t binding, BufferHandle buffer, size_t byte_offset, int32_t frame)
    {
        HB_ASSERT(frame < int32_t(MAX_FRAMES_IN_FLIGHT), "Frame index out of range");
        const VkDescriptorSetLayoutBinding& descriptorSetLayoutBinding = pipeline_layout->getDescriptorBindings()[binding];
        HB_ASSERT(descriptorSetLayoutBinding.descriptorType == VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, "binding#" + std::to_string(binding) + " is not a storage buffer");
        ...
        buffer_infos[write_index]->buffer = vk_buffer.getHandle();
    ...
    void VK_PipelineInstance::setTexelBuffer(uint32_t binding, TexelBufferHandle buffer, int32_t frame)
    {
        HB_ASSERT(frame < int32_t(MAX_FRAMES_IN_FLIGHT), "Frame index out of range");
        const VkDescriptorSetLayoutBinding& descriptorSetLayoutBinding = pipeline_layout->getDescriptorBindings()[binding];
    ...
    void VK_PipelineInstance::setTexelBufferArray(uint32_t binding, TexelBufferHandle* buffers, uint32_t buffer_count, int32_t frame)
    {
        HB_ASSERT(frame < int32_t(MAX_FRAMES_IN_FLIGHT), "Frame index out of range");
        const VkDescriptorSetLayoutBinding& descriptorSetLayoutBinding = pipeline_layout->getDescriptorBindings()[binding];
        HB_ASSERT(descriptorSetLayoutBinding.descriptorType == VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER, "binding#" + std::to_string(binding) + " is not a storage buffer");
