...
namespace HBE {
	void VK_Shader::alloc(VK_Context *context, const ShaderInfo &info) {
		...
		setSource(info.spirv, info.spirvLength);
	}

	void VK_Shader::release() {
		vkDestroyShaderModule(context->device.getHandle(), handle, nullptr);
		handle = VK_NULL_HANDLE;
	}

	bool VK_Shader::allocated() {
	...
	void VK_Shader::reflect(const uint32_t *spirv, uint32_t spirv_length) {
		...
		spirv_cross::CompilerGLSL glsl(spirv, spirv_length);
		...
		if (this->stage == SHADER_STAGE_COMPUTE) {
			...
			compute_workgroup_size.x = entry_point.workgroup_size.x;
			compute_workgroup_size.y = entry_point.workgroup_size.y;
