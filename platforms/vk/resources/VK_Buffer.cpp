...
namespace HBE {
	VkBuffer VK_Buffer::getVkHandle() const {
		return handle;
	}
	...
	void VK_Buffer::alloc(VK_Context *context, const BufferInfo &info) {
		...
		vk_buffer_info.usage |= (info.usage & BUFFER_USAGE_FLAG_INDEX) != 0 ? VK_BUFFER_USAGE_INDEX_BUFFER_BIT : 0;
		vk_buffer_info.usage |= (info.usage & BUFFER_USAGE_FLAG_VERTEX) != 0 ? VK_BUFFER_USAGE_VERTEX_BUFFER_BIT : 0;
		vk_buffer_info.usage |= (info.usage & BUFFER_USAGE_FLAG_STORAGE_BUFFER) != 0 ? VK_BUFFER_USAGE_STORAGE_BUFFER_BIT : 0;
		vk_buffer_info.usage |= (info.usage & BUFFER_USAGE_FLAG_TRANSFER_DST) != 0 ? VK_BUFFER_USAGE_TRANSFER_DST_BIT : 0;
		vk_buffer_info.usage |= (info.usage & BUFFER_USAGE_FLAG_TRANSFER_SRC) != 0 ? VK_BUFFER_USAGE_TRANSFER_SRC_BIT : 0;
		vk_buffer_info.usage |= (info.usage & BUFFER_USAGE_FLAG_UNIFORM) != 0 ? VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT : 0;
		vk_buffer_info.usage |= (info.usage & BUFFER_USAGE_FLAG_RAY_TRACING) != 0 ? VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT : 0;
		vk_buffer_info.size = info.stride * info.count;
	...
	void VK_Buffer::copy(VK_Buffer *other) {
		allocator->copy(other->getVkHandle(), this->getVkHandle(), size);
	}
