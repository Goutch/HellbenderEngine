...
namespace HBE {
	...
	class Group {
		...
		Group(std::vector<RegistryPage *> &pages, std::vector<ComponentTypeInfo> types) {
			for (int i = 0; i < types.size(); ++i) {
				components_flags.set(types[i].bit_number);
			}
			this->pages = pages;
		}
		...
		struct iterator {
			...
			iterator(std::vector<RegistryPage *> &pages, std::bitset<REGISTRY_MAX_COMPONENT_TYPES> flags, bool end):pages(pages) {
				this->current_entity = 0;
				this->components_flags = flags;
				finish = end;
				updatePages();
			}
		...
		public:
			...
			bool operator==(const iterator &other) const {
				return components_flags == other.components_flags &&
			...
			bool operator!=(const iterator &other) const {
				return current_entity != other.current_entity ||
					   components_flags != other.components_flags ||
					   finish != other.finish;
			}
			...
			void updatePages() {
				...
				if (current_page == pages.size()) {
					...
					current_pages = std::tuple<ComponentPage<Components>...>(pages[current_page]->getPage<Components>()...);
					size_t min_size_index = 0;
					for (size_t i = 1; i < sizeof...(Components); ++i) {
						if (std::get<min_size_index>(current_pages)->handles.size() > std::get<i>(current_pages)->handles.size()) {
							min_size_index = i;
						}
					}
					entities=std::get<min_size_index>(current_pages)->handles;
	...
	public:
		iterator begin() {
			return iterator(pages, components_flags, false);
		};

		iterator end() {
			return iterator(pages, components_flags, true);
		}
	};
	...
	public:
		...
		Group<Components...> group() {
			...
			return std::move(Group<Components...>());
		};
