...
namespace HBE {
	...
	Allocation &VK_Allocator::alloc(VkMemoryRequirements mem_requirements, AllocFlags flags) {
		...
		for (auto block_it = blocks[memory_type].begin(); block_it != blocks[memory_type].end(); ++block_it) {
			...
			for (auto allocation_it = block_it->allocations.begin(); allocation_it != block_it->allocations.end(); ++allocation_it) {
				...
				if (available >= mem_requirements.size) {
					...
					Log::debug("memory_type=" + std::to_string(memory_type) + "|position=" + std::to_string(position) + "|alignment=" + std::to_string(mem_requirements.alignment) + "|size=" + std::to_string(mem_requirements.size));
		...
		Log::debug("memory_type=" + std::to_string(memory_type) + "|new blocK!|alignment=" + std::to_string(mem_requirements.alignment) + "|size=" + std::to_string(mem_requirements.size));
