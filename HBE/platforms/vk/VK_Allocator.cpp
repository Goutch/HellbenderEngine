...
namespace HBE {
    ...
    std::string VK_Allocator::allocToString(const Allocation &alloc) {
        ...
        return std::to_string(alloc.size) + " bytes of " + memoryTypeToString(alloc.block.memory_type) +
    ...
    Allocation &VK_Allocator::alloc(VkMemoryRequirements mem_requirements, ALLOC_FLAGS flags) {
        ...
        for (auto block_it = blocks[memory_type].begin(); block_it != blocks[memory_type].end(); ++block_it) {
            ...
            for (auto allocation_it = block_it->allocations.begin();
                 allocation_it != block_it->allocations.end(); ++allocation_it) {
                ...
                if (available >= mem_requirements.size) {
                    ...
                    std::list<Allocation>::iterator new_alloc_it = block_it->allocations.emplace(allocation_it,
                                                                                                 Allocation{
            ...
            if (end - position >= mem_requirements.size) {
                ...
                block_it->remaining -= mem_requirements.size;
                const std::list<Allocation>::iterator &new_alloc_it = block_it->allocations.emplace(
                        block_it->allocations.end(),
                        Allocation{
                                .size=mem_requirements.size,
                                .offset=position,
                                .block=*block_it,
        ...
        Block &block = blocks[memory_type].emplace_back(Block{
                ...
                .index=index,
                .alloc_count=0,
                .remaining=(mem_requirements.size > BLOCK_SIZE ? mem_requirements.size : BLOCK_SIZE) -
    ...
    void VK_Allocator::setImageLayout(VK_Image *image, VkImageLayout newLayout) {
        ...
        //todo use transfer queue
        if (vkQueueSubmit(device->getGraphicsQueue(), 1, &submitInfo, image->getFence().getHandle()) != VK_SUCCESS) {
