...
namespace HBE {
	...
	VK_RenderPass::VK_RenderPass(VK_Renderer *renderer) {
		this->renderer = renderer;
		this->device = renderer->getDevice();
		this->height = renderer->getSwapchain().getExtent().height;
		this->width = renderer->getSwapchain().getExtent().width;
		this->clear_color = {0.f, 0.f, 0.f, 1.f};
		this->vk_format = renderer->getSwapchain().getFormat();
	...
	void VK_RenderPass::begin(VkCommandBuffer const &command_buffer, uint32_t i) const {
	...
	void VK_RenderPass::createFramebuffers() {
		...
		if (use_swapchain) {
			image_views = renderer->getSwapchain().getImagesViews();
		} else {
			images.resize(renderer->getSwapchain().getImagesViews().size());
	...
	const VK_Image *VK_RenderPass::getImage(uint32_t i) const {
		if(use_swapchain)
		{
			Log::error("Cant access swapchain images");
		}
		return images[i];
	}
