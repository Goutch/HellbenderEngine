...
namespace HBE {
    VK_PhysicalDevice::VK_PhysicalDevice(VkInstance vk_instance_handle, VkSurfaceKHR surface_handle) {
        ...
        vulkan_11_features.pNext = &synchronization2_features;
    ...
    bool VK_PhysicalDevice::isDeviceSuitable(VkPhysicalDevice const &physical_device) {
        ...
        bool required_extensions_support = checkExtensionsSupport(physical_device, REQUIRED_EXTENSIONS);
        if (required_extensions_support) {
            SwapchainSupportDetails swapchain_support_details = querySwapchainSupportDetails(physical_device);
            swapchain_support = !swapchain_support_details.present_modes.empty() && !swapchain_support_details.formats.empty();
        ...
        return queue_families_indices.isComplete() && required_extensions_support && swapchain_support;
    }
