...
namespace HBE {
    ...
    void VK_Renderer::render(const RenderTarget *render_target,
        ...
        device->getQueue(QUEUE_FAMILY_TRANSFER)->wait();
        ...
        render_pass->begin(current_frame);
        ...
        render_pass->end(current_frame);

        VkSemaphore waitSemaphores[] = {};
        VkPipelineStageFlags waitStages[] = {VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT};
        VkSemaphore signalSemaphores[] = {render_pass->getSemaphore(current_frame).getHandle()};
        device->getQueue(QUEUE_FAMILY_GRAPHICS)->submit(render_pass->getCommandBuffer(current_frame),
                                                        waitSemaphores,
                                                        waitStages,
                                                        1,
                                                        signalSemaphores,
    ...
    void VK_Renderer::present(const HBE::RenderTarget *render_target) {
        ...
        screen_pipeline->setTexture("texture0", current_render_pass);
        ...
        presentInfo.pWaitSemaphores = &frames[current_frame].finished_semaphore->getHandle();
